...
def step_coregister_t1_dwi(
        main_dax, config, relevant_t1_job, relevant_dwi_job):
    ...
    job1.addArguments(config.mri.t1_mgz_file, config.mri.t1_nii_file,
                      "--out_orientation", "RAS", "-rt", "nearest")
    job1.uses(config.mri.t1_mgz_file, link=Link.INPUT)
    job1.uses(config.mri.t1_nii_file, link=Link.OUTPUT,
              transfer=True, register=False)

    job2 = Job(
        name="flirt", node_label="Register DWI to T1 and get the relevant transform")
    ...
    job2.uses(config.mri.d2t_file, link=Link.OUTPUT,
              transfer=True, register=False)
    job2.uses(config.mri.b0_in_t1_file, link=Link.OUTPUT,
              transfer=True, register=False)

    job3 = Job(name="convert_xfm",
               node_label="Generate inverse transform from T1 to DWI")
    job3.addArguments("-omat", config.mri.t2d_file,
                      "-inverse", config.mri.d2t_file)
    job3.uses(config.mri.d2t_file, link=Link.INPUT)
    job3.uses(config.mri.t2d_file, link=Link.OUTPUT,
              transfer=True, register=False)

    job4 = Job(
        name="flirt",
        node_label="Apply inverse transform from T1 to DWI")
    ...
    job4.uses(config.mri.t1_in_d_file, link=Link.OUTPUT,
              transfer=True, register=False)
