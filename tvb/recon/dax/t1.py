...
from tvb.recon.dax.config import SubtypeConfiguration
...
def _step_recon_all(main_dax, previous_job, main_config, subtype_config):
    ...
    if subtype_config.is_dicom:
        ...
        job1 = Job(name="mri_convert", node_label="DICOM input pre-processing for " +
                   subtype_config.prefix.upper())
        ...
        job1.uses(subtype_config.raw_nii_file, link=Link.OUTPUT,
    ...
    job2 = Job(name="recon-all", node_label="Call 'recon-all' for " +
               subtype_config.prefix.upper())
...
def steps_recon_all(main_dax, config):
    ...
    mri_job.uses(config.mri.aseg_nii_file, link=Link.OUTPUT,
                 transfer=True, register=False)
