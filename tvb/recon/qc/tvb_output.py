...
from tvb.recon.dax.mappings import AsegFiles, OutputConvFiles, T1Files
...
def create_tvb_dataset(atlas_suffix: AtlasSuffix, mri_direc: os.PathLike,
    ...
    is_cortical_rm = numpy.genfromtxt(os.path.join(region_details_direc, AsegFiles.CORTICAL_TXT % atlas_suffix),
                                      usecols=[0], dtype='i')
    region_names = numpy.genfromtxt(os.path.join(region_details_direc, AsegFiles.CENTERS_TXT % atlas_suffix),
                                    usecols=[0], dtype="str")
    region_centers = numpy.genfromtxt(os.path.join(region_details_direc, AsegFiles.CENTERS_TXT % atlas_suffix),
                                      usecols=[1, 2, 3])
    region_areas = numpy.genfromtxt(os.path.join(region_details_direc, AsegFiles.AREAS_TXT % atlas_suffix), usecols=[0])
    region_orientations = numpy.genfromtxt(
        os.path.join(region_details_direc, AsegFiles.ORIENTATIONS_TXT % atlas_suffix), usecols=[0, 1, 2])
    rm_idx = numpy.genfromtxt(os.path.join(region_details_direc, AsegFiles.RM_TO_APARC_ASEG_TXT % atlas_suffix),
                              usecols=[0, 1], dtype='i')
    ...
    aparc_aseg_file = os.path.join(mri_direc, T1Files.APARC_ASEG_NII_GZ % atlas_suffix)
    ...
    IOUtils.write_volume(os.path.join(out_dir, OutputConvFiles.APARC_ASEG_COR_NII_GZ % atlas_suffix),
