...
def compute_region_details(atlas: AtlasSuffix, fs_color_lut: os.PathLike, t1: os.PathLike, lh_cort: os.PathLike,
    ...
    genericIO.write_list_to_txt_file(mapping.cort_region_mapping, AsegFiles.RM_CORT_TXT % atlas)
    genericIO.write_list_to_txt_file(mapping.subcort_region_mapping, AsegFiles.RM_SUBCORT_TXT % atlas)
    ...
    genericIO.write_dict_to_txt_file(dict_fs_custom, AsegFiles.FS_CUSTOM_TXT % atlas)
    ...
    genericIO.write_list_to_txt_file(region_areas, AsegFiles.AREAS_TXT % atlas)
    ...
    with open(AsegFiles.CENTERS_TXT % atlas, "w") as f:
    ...
    lh_region_orientations = surface_service.compute_orientations_for_regions(mapping.get_lh_regions(), surf_cort_lh,
                                                                              mapping.lh_region_mapping)
    with open(AsegFiles.LH_DIPOLES_TXT % atlas, "w") as f:
    ...
    rh_region_orientations = surface_service.compute_orientations_for_regions(mapping.get_rh_regions(), surf_cort_rh,
                                                                              mapping.rh_region_mapping)
    with open(AsegFiles.RH_DIPOLES_TXT % atlas, "w") as f:
    ...
    numpy.savetxt(AsegFiles.ORIENTATIONS_TXT % atlas, region_orientations, fmt='%.2f %.2f %.2f')
    ...
    genericIO.write_dict_to_txt_file(rm_index_dict, AsegFiles.RM_TO_APARC_ASEG_TXT % atlas)

    genericIO.write_list_to_txt_file(mapping.is_cortical_region_mapping(), AsegFiles.CORTICAL_TXT % atlas)
