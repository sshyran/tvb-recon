...
then
    ...
    #python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.vol_to_ext_surf_vol('$MRI/$vol.nii.gz',labels='$ASEG_LIST',ctx='lh rh',out_vol_path='./$vol-surf.nii.gz',labels_surf=None,labels_inner='0')"
...
#Using my python code:
#python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.mask_to_vol('$mask_this_vol.nii.gz','./mask-$SEGMENT_METHOD.nii.gz','./$vol-mask.nii.gz',labels='$ASEG_LIST',ctx='lh rh',vol2mask_path=None,vn=$VOL_VN,th=1,labels_mask=None,labels_nomask='0')"
...
for h in lh rh
do
    python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.sample_vol_on_surf('$SURF/$h.white','./$vol-mask.nii.gz','$LABEL/$h.aparc.annot','./$h.white-mask','$CRAS_PATH',add_string='ctx-'+'$h'+'-',vertex_neighbourhood=int('$SURF_VN'), add_lbl=[2,41], lut_path='$FREESURFER_HOME/FreeSurferColorLUT.txt')"
done
#Sub-cortical:
for h in lh rh
do
    python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.sample_vol_on_surf('$SURF/$h.aseg','./$vol-mask.nii.gz','$LABEL/$h.aseg.annot','./$h.aseg-mask','$CRAS_PATH',vertex_neighbourhood=int('$SURF_VN'),lut_path='$FREESURFER_HOME/FreeSurferColorLUT.txt')"
done
...
then
    ...
    python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.node_connectivity_metric('$DMR/vol-counts$STRMLNS_SIFT_NO-v$VOX.npy',metric='cosine', out_consim_path='$out_consim_path')"
