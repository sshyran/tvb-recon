...
python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.aseg_surf_conc_annot('./aseg_filled','$ASEG_LIST','$FREESURFER_HOME/FreeSurferColorLUT.txt')"
python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.aseg_surf_conc_annot('./aseg_filled-ras','$ASEG_LIST','$FREESURFER_HOME/FreeSurferColorLUT.txt')"

#Get the voxels that lie at the external surface of the aseg structures:
python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.vol_to_ext_surf_vol('./aseg_filled.nii','$ASEG_LIST',out_vol_path='./aseg_filled_surf.nii',labels_surf='$ASEG_LIST',labels_inner='0')"
...
then
    ...
    python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.mask_to_vol('../aseg_filled_surf.nii','./gwi_mask.nii','../aseg_filled_surf_con_gwi.nii','$ASEG_LIST',vol2maskTrnsfrmPath=None,vn=$ASEG_GWI_VN,th=1,labels_mask='$ASEG_LIST',labels_nomask='0')"
...
then
    ...
    python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.mask_to_vol('../aseg_filled_surf.nii','./tdi_mask.nii','../aseg_filled_surf_con_tdi.nii','$ASEG_LIST',vol2maskTrnsfrmPath='$DMR/t2d.mat',vn=$ASEG_TDI_VN,th=1,labels_mask='$ASEG_LIST',labels_nomask='0')"
...
do
    ...
    echo $ASEG_CON_SURF
    python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.sample_vol_on_surf('./aseg_filled','$aseg','$ASEG_CON_SURF','$ASEG_LIST', vn=$SURF_VN)"

    #Concatenate surfaces and create annotation:
    python -c "import tvb.recon.algo.reconutils; tvb.recon.algo.reconutils.aseg_surf_conc_annot('$ASEG_CON_SURF','$ASEG_LIST','$FREESURFER_HOME/FreeSurferColorLUT.txt')"
done
